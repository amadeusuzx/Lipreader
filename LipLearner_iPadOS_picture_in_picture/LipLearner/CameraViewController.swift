...
extension CameraViewController: GCDAsyncSocketDelegate{
    ...
    func socket(_ sock: GCDAsyncSocket, didAcceptNewSocket newSocket: GCDAsyncSocket) {
        ...
        let newClient = StreamingSession(client: newSocket, queue: DispatchQueue.global(qos: .userInteractive), userName: "zxsu")
        self.clientSocket = newClient
        newClient.socketStartStreaming()
    }
}



final class CameraViewController: UIViewController, SFSpeechRecognizerDelegate, ItemSelectionViewControllerDelegate, AVPictureInPictureControllerDelegate {
    ...
    private var pipPreviewView: PreviewView!
    private var captureDevice: AVCaptureDevice?
    private var captureDeviceResolution: CGSize = CGSize(width: 1920, height: 1080)
    ...
    private var keywordThreshold: Float = 0.58
    private var silentCenterVector = [Float]()
    private var silentCount: Float = 0
    private var silentThreshold: Float = 0.63
    ...
    private var clientSocket: StreamingSession?
    ...
    override func viewDidLoad() {
        ...
        self.serverSocket = GCDAsyncSocket(delegate: self, delegateQueue: DispatchQueue.global(qos: .userInteractive), socketQueue: DispatchQueue.global(qos: .userInteractive))
        print("Created a socket server with ip: \(String(describing: ipAddress))")
        do {
            try self.serverSocket?.accept(onInterface: ipAddress, port: 10001)
        } catch {
    ...
    @objc func appEnteredForeground() {
        pipVideoCallViewController?.view.subviews.last?.removeFromSuperview()
//        pipPreviewView.isHidden = false
        pipPreviewView.frame = cameraView.frame
        cameraView.addSubview(self.pipPreviewView)
        NSLayoutConstraint.activate([
            pipPreviewView.topAnchor.constraint(equalTo: cameraView.topAnchor),
            pipPreviewView.leadingAnchor.constraint(equalTo: cameraView.leadingAnchor),
            pipPreviewView.trailingAnchor.constraint(equalTo: cameraView.trailingAnchor),
            pipPreviewView.bottomAnchor.constraint(equalTo: cameraView.bottomAnchor)
        ])
        let scaleY = pipPreviewView.previewLayer.bounds.height / captureDeviceResolution.height
        var affineTransform = CGAffineTransform.identity
        affineTransform = affineTransform.scaledBy(x: scaleY, y: -scaleY)
        detectionOverlayLayer?.setAffineTransform(affineTransform)
        detectedFaceLandmarksShapeLayer?.lineWidth = 3
        detectedFaceRectangleShapeLayer?.lineWidth = 5
    }
    
    @objc func appMovedToBackground() {
        print("App moved to background!")
        pipPreviewView.frame = pipVideoCallViewController!.view.frame
//        pipPreviewView.isHidden = true
        pipVideoCallViewController?.view.addSubview(pipPreviewView)
        
        NSLayoutConstraint.activate([
            pipPreviewView.topAnchor.constraint(equalTo: pipVideoCallViewController!.view.topAnchor),
            pipPreviewView.leadingAnchor.constraint(equalTo: pipVideoCallViewController!.view.leadingAnchor),
            pipPreviewView.trailingAnchor.constraint(equalTo: pipVideoCallViewController!.view.trailingAnchor),
            pipPreviewView.bottomAnchor.constraint(equalTo: pipVideoCallViewController!.view.bottomAnchor)
        ])
        let scaleY = pipPreviewView.previewLayer.bounds.height / captureDeviceResolution.height
        var affineTransform = CGAffineTransform.identity
        affineTransform = affineTransform.scaledBy(x: scaleY, y: -scaleY)
        affineTransform = affineTransform.translatedBy(x: -1194/1.94, y: 834/1.94)
        affineTransform = affineTransform.scaledBy(x: 0.2, y: 0.2)
        detectionOverlayLayer?.setAffineTransform(affineTransform)
        detectedFaceLandmarksShapeLayer?.lineWidth = 7
        detectedFaceRectangleShapeLayer?.lineWidth = 10
    }
        ...
        alert.addAction(UIAlertAction(title: "Load Data", style: .default, handler: { [weak alert] (_) in
            ...
            do {
                ...
                if let commandCenterDict = NSKeyedUnarchiver.unarchiveObject(withFile: datFileURL.path){
                    ...
                    if let silentCenterVector = self.commandCenterDict.removeValue(forKey: "silent"){
                        self.silentCenterVector = silentCenterVector
                        self.silentCount = self.silentCenterVector.remove(at: 0)
                    }
    ...
    func appendToDataFrame(){
        ...
        if commandCenterDict[cmd] == nil {
            ...
            var newVector = commandCenterDict[cmd]!.suffix(from: 1).map(){$0 * commandCenterDict[cmd]![0]} // restore to sum of all silent vectors
    ...
    @IBAction func KWSSwitchToggled(_ sender: UISwitch) {
        if sender.isOn{
            if self.silentCount > 0 && self.keywordCount > 0{
                self.keywordSpotting = true
            }else{
                sender.setOn(false, animated: true)
                let alert = UIAlertController(title: "Alert", message: "Record keyword and silent samples first!", preferredStyle: .alert)
    ...
    @IBAction private func recordReleased(_ sender: UIButton, forEvent event: UIEvent) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if self.currentWorkMode == .registeration && self.currentRecordMode == .silent {
            ...
            if let featureVector = self.featureExtraction(modelInput: self.modelInput){
                ...
                case .registeration:
                    ...
                    case .keyword:
                        ...
                        print("------------keyword count:", self.keywordCount, "--------------")
                        print("print(self.keywordCount", self.keywordCount)
                    case .silent:
                        let result = self.addNewKWSVector(vector: self.silentCenterVector, count: self.silentCount, featureVector: featureVector)
                        self.silentCenterVector = result.0
                        self.KWSDataFrame.addLipData(command: "N", vector: self.silentCenterVector)
                        self.silentCount = result.1
                        print("self.silentCount", self.silentCount)
                    case .command:
...
//                            self.workspace.open(URL(string: shortcutURLString)!)
//                            self.workspace.openApplication(withBundleID: "com.paperpile.paperpile")
                            self.clientSocket?.client.write(classificationResult.data(using: .utf8), withTimeout: -1, tag: 0)
                        }else{
//                            let shortcutURLString =  "shortcuts://run-shortcut?name=" + classificationResult.replacingOccurrences(of: " ", with: "%20")
//                            self.workspace.openApplication(withBundleID: "com.zxsu.LipLearner")
//                            self.workspace.open(URL(string: shortcutURLString)!)
                            if classificationResult.contains("slides"){
                                self.workspace.openApplication(withBundleID: "com.google.Slides")
                            } else {
                                self.clientSocket?.client.write(classificationResult.data(using: .utf8), withTimeout: -1, tag: 0)
                            }
//                            UIApplication.shared.open(URL(string: shortcutURLString)!)
                        }
//                        self.pipPreviewView.isHidden = true
                    }
            ...
            self.modelInput = [UIImage]()
            self.detectedFaceRectangleShapeLayer?.strokeColor = UIColor.green.withAlphaComponent(0.7).cgColor
            DispatchQueue.main.async {
    ...
    private func addNewKWSVector(vector: [Float], count: Float, featureVector: [Float]) -> ([Float], Float){
        if count == 0{
            ...
            var newVector = vector.map(){$0*count} // restore to sum of all silent vectors
    ...
    private func setUpUI(){
        ...
        let menuButtonActions = [UIAction(title: "Save and Train", state: .off, handler: { _ in self.saveDataAndTrain()}),]
//                                 UIAction(title: "Reset keyword", state: .off, handler: { _ in
//            let alert = UIAlertController(title: "Confirm", message: "Are you sure to reset the keyword and the silent samples?", preferredStyle: .alert)
//            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { (_) in
//                self.keywordCount = 0
//                self.silentCount = 0
//                self.keywordCenterVector = [Float]()
//                self.silentCenterVector = [Float]()
//            }))
...
//        })]
        ...
        let actions = [UIAction(title: "Keyword", state: .on, handler: {_ in self.currentRecordMode = .keyword}),
                       UIAction(title: "Non speaking", state: .off, handler: {_ in self.currentRecordMode = .silent}),
        ...
        let rootLayer = pipPreviewView.previewLayer
        ...
        let scaleY = rootLayer.bounds.height / captureDeviceResolution.height  //* 1.25
//        let scaleY = 0.5
        var affineTransform = CGAffineTransform.identity
        affineTransform = affineTransform.scaledBy(x: scaleY, y: -scaleY)
    ...
    fileprivate func addIndicators(to faceRectanglePath: CGMutablePath, faceLandmarksPath: CGMutablePath, for faceObservation: VNFaceObservation) {
        ...
        if let landmarks = faceObservation.landmarks {
            ...
            faceRectanglePath.addRect(lipCenteredFaceBounds)
...
//            previewLayer.frame = CGRect(x: 0, y: 0, width: self.view.bounds.width, height: self.view.bounds.height)
            ...
            pipVideoCallViewController = .init()

            pipVideoCallViewController!.preferredContentSize = CGSize(width: 1194, height: 834)
            ...
            pipPreviewView = PreviewView(captureSession)
            captureSession.connections.last?.videoOrientation = .landscapeRight
            pipPreviewView.previewLayer.videoGravity = .resizeAspectFill
            pipPreviewView.translatesAutoresizingMaskIntoConstraints = false
            pipPreviewView.frame = cameraView.frame
            cameraView.addSubview(pipPreviewView) // size : (414.0, 844.0)
            NSLayoutConstraint.activate([
                pipPreviewView.topAnchor.constraint(equalTo: cameraView.topAnchor),
                pipPreviewView.leadingAnchor.constraint(equalTo: cameraView.leadingAnchor),
                pipPreviewView.trailingAnchor.constraint(equalTo: cameraView.trailingAnchor),
            ...
            videoOutput.connections.last?.videoOrientation = .landscapeRight
    ...
    func keywordDetection(video: [UIImage]){
        ...
        if !self.recording{  // not recording, need to detect keyword
            ...
            if sim > self.keywordThreshold{ // otherwise the data is consifered as positive candidates.
                ...
                DispatchQueue.main.async {
//                    self.pipPreviewView.isHidden = false
                    self.detectedFaceRectangleShapeLayer?.strokeColor = UIColor.red.withAlphaComponent(0.7).cgColor
...
//            self.windowCount += 1
            if self.modelInput.count > 45 { // skip the possible silent immediately after the keyword
//                self.windowCount = 0
                var sim: Float = .nan
                vDSP_dotpr(keywordVector, 1, self.silentCenterVector, 1, &sim, vDSP_Length(500))
                var norm: Float = .nan
                vDSP_svesq(self.silentCenterVector, 1, &norm, vDSP_Length(500))
                sim *= norm.squareRoot()
                
                print("Similarity: ", sim, "threshold:", self.silentThreshold)
                if sim > self.silentThreshold { // 1.5s elapsed after keyword feedback{
                    DispatchQueue.main.async {
                        self.recordButton.sendActions(for: .touchUpInside)
                    }
                }
            }
//            if self.windowCount > 1 { // skip one hop to avoid overlapping with the keyword utterance
//                self.kwsDataFrame.addLipData(command: "N", vector: keywordVector)
//                if (self.kwsDataFrame.columns[0].filter{($0 as! String) == "N"}).count < 100 {
//                    self.kwsDataFrame.addLipData(command: "N", vector: keywordVector)
//                }
//            }
        }
    ...
    private func saveDataAndTrain(){
        ...
        DispatchQueue.global(qos: .userInteractive).async {
            ...
            do {
                self.KWSClassifier = self.trainClassifier(dataFrame: self.KWSDataFrame)
//                let tok = UInt64((Date().timeIntervalSince1970) * 1000)
                self.logisticRegressionClassifier = self.trainClassifier(dataFrame: self.trainDataFrame)
//                print(UInt64(Date().timeIntervalSince1970) * 1000 - tok)
                let path = try fileManager.url(for: .documentDirectory, in: .allDomainsMask, appropriateFor: nil, create: false)
                ...
                dictData["silent"] = self.silentCenterVector
                dictData["silent"]!.insert(self.silentCount, at: 0)
...
extension CameraViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from _: AVCaptureConnection) {
        //
        //        let tok = UInt64((Date().timeIntervalSince1970) * 1000)
        //        if (tok - self.fpsTik) < 30 || self.recognizing{
        //            return
        //        }else{
        //            self.fpsTik = tok
        //        }
        ...
        let transform = sourceImage.orientationTransform(for: .downMirrored).inverted()
        ...
        let image = UIImage(cgImage: tempImage, scale:1.0, orientation: .downMirrored)
